...
def to_one_hot(seqlist, max_length=None, alphabet=aa_letters):
    aa_key = {l: i for i, l in enumerate(alphabet)}
    if isinstance(seqlist, str):  # If a single sequence
        seqlist = [seqlist]

    encoded_seqs = []
    for prot in seqlist:
        encoded_seq = seq_to_one_hot(prot, aa_key)
        if max_length is not None:
            padded_seq = right_pad([encoded_seq], max_length)
            encoded_seqs.append(padded_seq[0])
        else:
